//
// AUTO-GENERATED FILE, DO NOT MODIFY!
//
// @dart=2.12

// ignore_for_file: unused_element, unused_import
// ignore_for_file: always_put_required_named_parameters_first
// ignore_for_file: constant_identifier_names
// ignore_for_file: lines_longer_than_80_chars

import 'package:ventimetri_api/api.dart';
import 'package:test/test.dart';


/// tests for BranchControllerApi
void main() {
  // final instance = BranchControllerApi();

  group('tests for BranchControllerApi', () {
    //Future confirmEmployee(String branchCode, String userCode, String adminUserCode) async
    test('test confirmEmployee', () async {
      // TODO
    });

    //Future<Object> delete(String branchCode) async
    test('test delete', () async {
      // TODO
    });

    //Future<BranchResponseEntity> getBranchDataByBranchCode(String branchCode) async
    test('test getBranchDataByBranchCode', () async {
      // TODO
    });

    //Future<BranchResponseEntity> getBranchDataByBranchCodeAndUserCode(String userCode, String branchCode) async
    test('test getBranchDataByBranchCodeAndUserCode', () async {
      // TODO
    });

    //Future<List<BranchResponseEntity>> getBranchDataByBranchType(String branchType) async
    test('test getBranchDataByBranchType', () async {
      // TODO
    });

    //Future<List<EmployeeEntity>> getEmployeeByBranchCode(String branchCode) async
    test('test getEmployeeByBranchCode', () async {
      // TODO
    });

    //Future<List<EmployeeEntity>> getEmployeeByUserCode(String userCode) async
    test('test getEmployeeByUserCode', () async {
      // TODO
    });

    //Future<VentiMetriQuadriData> linkUserToBranch(String userName, String userCode, List<String> branchCodes, String role, String fcmToken) async
    test('test linkUserToBranch', () async {
      // TODO
    });

    //Future<Object> removeUserFromBranch(String userCode, String branchCode) async
    test('test removeUserFromBranch', () async {
      // TODO
    });

    //Future<VentiMetriQuadriData> retrieveAll() async
    test('test retrieveAll', () async {
      // TODO
    });

    //Future<BranchResponseEntity> retrieveBranchByUserCodeAndBranchCode(String userCode, String branchCode) async
    test('test retrieveBranchByUserCodeAndBranchCode', () async {
      // TODO
    });

    //Future<CounterEntity> retrieveBranchCounters(String branchCode) async
    test('test retrieveBranchCounters', () async {
      // TODO
    });

    //Future<VentiMetriQuadriData> retrieveBranches(String userCode, String branchType, { String fcmToken }) async
    test('test retrieveBranches', () async {
      // TODO
    });

    //Future<VentiMetriQuadriData> retrieveData(String userCode, { String fcmToken }) async
    test('test retrieveData', () async {
      // TODO
    });

    //Future<BranchResponseEntity> save(BranchCreationEntity branchCreationEntity) async
    test('test save', () async {
      // TODO
    });

    //Future setFcmToken(String userCode, String branchCode, String fcmToken) async
    test('test setFcmToken', () async {
      // TODO
    });

    //Future<Object> updateBranchData(BranchCreationEntity branchCreationEntity) async
    test('test updateBranchData', () async {
      // TODO
    });

    //Future updateEmployeeRole(String branchCode, String userCode, String role) async
    test('test updateEmployeeRole', () async {
      // TODO
    });

  });
}
