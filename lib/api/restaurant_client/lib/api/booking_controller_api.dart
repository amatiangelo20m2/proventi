...
class BookingControllerApi {
  ...
  /// Performs an HTTP 'GET /api/booking/retrievebooking/{bookingCode}' operation and returns the [Response].
  /// Parameters:
  ///
  /// * [String] bookingCode (required):
  Future<Response> retrieveBookingByCodeWithHttpInfo(String bookingCode,) async {
    // ignore: prefer_const_declarations
    final path = r'/api/booking/retrievebooking/{bookingCode}'
      .replaceAll('{bookingCode}', bookingCode);
  ...
  ///
  /// * [String] bookingCode (required):
  Future<BookingDTO?> retrieveBookingByCode(String bookingCode,) async {
    final response = await retrieveBookingByCodeWithHttpInfo(bookingCode,);
    if (response.statusCode >= HttpStatus.badRequest) {
      throw ApiException(response.statusCode, await _decodeBodyBytes(response));
    }
    // When a remote server returns no body with a status of 204, we shall not decode it.
